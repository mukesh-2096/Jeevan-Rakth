...
export default function BloodHealthInfo({ formData, updateFormData, errors }: BloodHealthInfoProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">
          Are you currently on any medication? <span className="text-red-600">*</span>
        </label>
        <div className="flex gap-4 mb-3">
          {['Yes', 'No'].map((option) => (
            <label key={option} className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                value={option}
                checked={formData.currentMedication === option}
      ...
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">
          Do you have any major illness? <span className="text-red-600">*</span>
        </label>
        <div className="flex gap-4 mb-3">
          {['Yes', 'No'].map((option) => (
            <label key={option} className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="majorIllness"
                value={option}
                checked={formData.majorIllness === option}
                onChange={(e) => updateFormData('majorIllness', e.target.value)}
                className="w-4 h-4 text-red-600 cursor-pointer"
              />
              <span className="text-sm text-gray-700">{option}</span>
            </label>
        ...
        {formData.majorIllness === 'Yes' && (
          <input
            ...
            onChange={(e) => updateFormData('illnessDetails', e.target.value)}
            placeholder="Please specify illness details"
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none transition text-sm text-gray-900 placeholder-gray-400"
          />
        )}
      </div>
